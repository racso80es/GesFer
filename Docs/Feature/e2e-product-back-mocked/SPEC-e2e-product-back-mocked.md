...
- [x] Existe una suite E2E ejecutable que testea al menos un flujo representativo del Product Back (p. ej. login + al menos un recurso: user o company).
- [x] Las dependencias del Product Back están mockeadas; los E2E pasan sin API real ni BD real (p. ej. con `USE_MOCK_API=1` y mock en 5002, o con TestServer + mocks).
- [x] El código compila sin errores (`dotnet build` en Back si se añade código C#; `npm run build` en Front si solo se toca Front).
- [x] Los tests E2E añadidos/modificados pasan localmente (Playwright y/o dotnet test según implementación).
- [x] Documentación actualizada (README de tests o `docs/infrastructure/`) indicando cómo ejecutar E2E para Product Back con mocks.
