...
/// <summary>
/// Implementación de Product para la aplicación de migraciones pendientes.
/// </summary>
public class ProductMigrationService : IMigrationService
{
    ...
    public async Task ApplyMigrationsAsync()
    {
        try
        {
            ...
            // Guard: Verificar que el proveedor sea relacional
            ...
            if (pendingMigrationsList.Any())
            {
                ...
                try
                {
                    ...
                    _logger.LogInformation("Migraciones aplicadas correctamente. Migraciones: {Migrations}", migrationsList);
                }
                catch (Exception migrateEx)
                {
                        ...
                        (migrateEx.InnerException?.Message?.Contains("already exists", StringComparison.OrdinalIgnoreCase) == true))
                    {
                        ...
                        {
                            _logger.LogInformation("Las migraciones ya están aplicadas. Migraciones: {Migrations}", string.Join(", ", appliedMigrationsList));
                        }
                        else
                        {
                            ...
                            catch (Exception fixEx)
                            {
                                ...
                                throw new InvalidOperationException(
                                    $"Error al aplicar migraciones: {migrateEx.Message}. Verifique la configuración de la base de datos.",
                                    migrateEx);
                            }
                        }
                    }
                    ...
                    {
                        _logger.LogError(migrateEx, "Error al aplicar migraciones. Tipo: {ExceptionType}, Mensaje: {Message}",
                            migrateEx.GetType().Name, migrateEx.Message);
                        throw new InvalidOperationException(
                            $"Error al aplicar migraciones: {migrateEx.Message}. Verifique la configuración de la base de datos.",
                            migrateEx);
                    }
