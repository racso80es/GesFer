...
public class MigrationService : IMigrationService
{
    ...
    public MigrationService(ProductDbContext context, ILogger<MigrationService> logger)
    {
        _context = context;
        _logger = logger;
    }
                        ...
                        (migrateEx.InnerException?.Message?.Contains("already exists", StringComparison.OrdinalIgnoreCase) == true))
                    {
                        ...
                        else
                        {
                            ...
                            catch (Exception fixEx)
                            {
                                ...
                                throw new InvalidOperationException($"Error al aplicar migraciones: {migrateEx.Message}.", migrateEx);
                            }
                        }
                    }
                    else
                    {
                        _logger.LogError(migrateEx, "Error al aplicar migraciones. Tipo: {ExceptionType}, Mensaje: {Message}", migrateEx.GetType().Name, migrateEx.Message);
                        throw new InvalidOperationException($"Error al aplicar migraciones: {migrateEx.Message}.", migrateEx);
                    }
                }
            }
