...
using Microsoft.Extensions.DependencyInjection; // For IServiceScope if needed, or GetService
...
public class ProductIntegrityService : IIntegrityCheckService
{
    ...
    private readonly IAdminApiClient? _adminClient; // Nullable as per original logic
    ...
    public ProductIntegrityService(
        ...
        IServiceProvider serviceProvider) // Inject SP to resolve nullable IAdminApiClient if not registered in all contexts?
    {
        _context = context;
        _logger = logger;
        _sanitizer = sanitizer;
        _environment = environment;
        // Resolve IAdminApiClient optionally
        _adminClient = serviceProvider.GetService<IAdminApiClient>();
    }
