...
process.env.VITE_PUBLIC = require$$1$2.app.isPackaged ? process.env.DIST : path.join(process.env.DIST, "../public");
let win;
let tray;
...
const schema = {
  ...
  servicePaths: { type: "object", default: {} }
};
const store = new Store({ schema });
function getIconPath(state) {
  const iconName = `icon-${state}.png`;
  return path.join(process.env.VITE_PUBLIC, "tray", iconName);
}
function createWindow() {
  win = new require$$1$2.BrowserWindow({
    width: 900,
    height: 600,
    icon: getIconPath("grey"),
...
function createTray() {
  const icon = require$$1$2.nativeImage.createFromPath(getIconPath("grey"));
  tray = new require$$1$2.Tray(icon);
  tray.setToolTip("Calma Desktop - Stopped");
  const contextMenu = require$$1$2.Menu.buildFromTemplate([
    { label: "Start Sequence 1 (Product)", click: () => {
      console.log("Start Seq 1");
    } },
    { label: "Stop All", click: () => {
      console.log("Stop All");
    } },
    { type: "separator" },
    { label: "Open Dashboard", click: () => win == null ? void 0 : win.show() },
    { type: "separator" },
    {
      label: "Quit Calma",
      click: () => {
...
require$$1$2.app.whenReady().then(() => {
  createWindow();
  createTray();
  require$$1$2.ipcMain.handle("get-settings", () => store.store);
  require$$1$2.ipcMain.handle("update-settings", (_, key, value) => {
    store.set(key, value);
    return store.store;
  });
  require$$1$2.ipcMain.on("app-quit", () => {
    isQuitting = true;
    require$$1$2.app.quit();
  });
  require$$1$2.ipcMain.handle("start-sequence", (_, seqId) => {
    console.log("IPC: start-sequence", seqId);
  });
  require$$1$2.ipcMain.handle("stop-all", () => {
    console.log("IPC: stop-all");
  });
  require$$1$2.ipcMain.handle("run-audit", () => {
    console.log("IPC: run-audit");
  });
  require$$1$2.ipcMain.handle("clear-cache", () => {
    console.log("IPC: clear-cache");
  });
  require$$1$2.ipcMain.handle("sync-spec", () => {
