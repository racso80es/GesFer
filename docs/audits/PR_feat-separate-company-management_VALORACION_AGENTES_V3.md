...
| **Responsable de la carga (runtime)** | Product | Solo **Product** ejecuta el seed de companies: `JsonDataSeeder.SeedCompaniesAsync()` en Product.Infrastructure. Usa `_context.Companies` del **ProductDbContext** (Product) e instancia `GesFer.Product.Back.Domain.Entities.Company`. Admin **no** tiene ningún seed de companies: `AdminJsonDataSeeder` solo carga `admin-users.json`; no existe `companies.json` ni método equivalente en Admin. |
| **Modelo y tabla** | Compartido | Admin tiene `AdminDbContext.Companies` (entidad `Shared.Back.Domain.Entities.Company`) mapeada a tabla `"Companies"`. Product tiene `ProductDbContext.Companies` (entidad que hereda de Shared.Company). Si ambos contextos apuntan a la **misma base de datos**, la tabla `Companies` es única; los datos insertados por Product al ejecutar su seed son visibles para Admin. |
