...
| :--- | :--- | :--- |
| **Violaciones de Terminología ("empresa")** | **32** | 🔴 CRÍTICO |
| **Integridad de Dependencias (Lockfiles)** | **True** | 🟢 PASA |
| **Deuda Técnica (`any`)** | **2** | 🟢 PASA |
...
- **src/Product/Front/messages/es.json**
  - Line 272: `"updateError": "Error al actualizar la empresa"...`
- **src/Product/Front/tests/page-objects/CompaniesPage.ts**
  - Line 15: `this.title = page.getByRole('heading', { name: /mi organización|organizaciones|empresas/i }).first()...`
- **src/Product/Front/lib/legacy-constants.ts**
  - Line 1: `export const DEMO_COMPANY_NAME = "Empresa Demo";...`
  - Line 2: `export const ADMIN_COMPANY_NAME = "Empresa Admin";...`
  - Line 3: `export const CLIENT_COMPANY_NAME = "Empresa Cliente";...`
  - ... y 1 más.
...
- **src/Admin/Front/tests/admin-e2e-mocked.spec.ts**
  - Line 37: `await expect(page.getByText('Empresa Demo 1')).toBeVisible();...`
  - Line 41: `await page.fill('input#name', 'Nueva Empresa Mock');...`
- **src/Admin/Front/tests/mock-api.js**
  - Line 29: `name: 'Empresa Demo 1',...`
  - Line 38: `name: 'Empresa Demo 2',...`
